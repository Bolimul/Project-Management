...
export const PersonalInfo = (props) => {
  ...
  const [workplaceName, setWorkplaceName] = useState(
    ExampleAccount.workplaceName
  );
  const [licenseNumber, setLicenseNumber] = useState(
    ExampleAccount.licenceNumber
  );
  const [OrganizationName, setOrganizationName] = useState(
    ExampleAccount.organisationName
  );
  const [OrganizationAdress, setOrganizationAdress] = useState(
    ExampleAccount.organisationAddress
  );
  const [OrganizationPhone, setOrganizationPhone] = useState(
    ExampleAccount.organisationPhone
  );
  const [StudentNumber, setStudentNumber] = useState(
    ExampleAccount.studentNumber
  );
  const [UniversityName, setUniversityName] = useState(
    ExampleAccount.universityName
  );
  ...
  const setDataInfo = async () => {
    var user = (
      await getDoc(doc(db, "users-profile-data", auth.currentUser.uid))
    ).data().personalInfo;
    // onSnapshot(doc(db,"users-profile-data",auth.currentUser.uid), (doc)=> {
    //   user = doc.data().personalInfo;
    //})
